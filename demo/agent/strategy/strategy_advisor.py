...

{
  ...
  "principles": [<string>, ...],      // 从今天的数据中提炼2-3条策略原则（抽象规律，非具体数据）
  "reasoning": "<一句话解释调整理由>"
}

关于 principles 字段：
- 提炼的是抽象策略原则，而非具体数据（如"夜间长途单净收益显著优于短途"而非"昨天收入800"）
- 每条原则应具有泛化性，能在后续多天指导决策
- 结合当天的具体表现总结出规律性认知
- 如果提供了 existing_principles（已有原则），可选择保留、修正或替换
...
12. 如果 opro_experience 中包含"失败实验"，这些是导致低收入或高罚分的参数组合，应当避免使用类似参数。
13. 如果 opro_experience 中包含"违规反思"，说明之前在某些场景下出现了约束违规，
    今日策略应针对性地调整参数以避免相同违规再次发生。
...
【第一步：反思当前状态】
在决策前，先回答以下问题（写在 reflection 字段中）：
- 当前月度进度如何？距月底还有多少天？累计净收入是否达标？
- 今天已接几单？是否还有时间窗口完成更多订单？
- 当前偏好约束的满足情况：休息够了吗？off-day进度正常吗？有无违规风险？
- 空驶预算还剩多少？当前候选的取货距离是否会严重消耗预算？
...
- 考虑空驶预算消耗情况
- 如果提供了 historical_experience（同时段同区域历史数据），参考历史平均收入判断：
  高于均值的货源优先接单，低于均值的可以等待更好机会
- 如果提供了 top_historical_decisions（该时段区域置信度最高的 top-3 历史决策），按置信度权衡参考：
  置信度高的经验更值得信赖，但不应完全忽略低置信度的多样性选择
- 如果提供了 delivery_region_experience（卸货区域历史数据），优先选卸货区域
  后续接单快（avg_wait低）且收入高的货源
- 如果提供了 strategic_principles（策略原则），这是从历史运营中蒸馏出的抽象规律，
  决策时应遵循这些原则，除非当前场景明确不适用
- 如果提供了 semantic_memory（语义记忆），这是从大量决策数据中统计提炼的客观规律
  （时段收益模式、黄金区域、等待策略有效性），应作为决策的重要参考依据

输出格式：
{
  "reflection": "<2-3句话的状态反思，锚定当前真实状态与目标差距>",
...
class ExperienceBuffer:
    ...
    R9-F 增强：新增 _worst_buffer 保留 worst-3 失败实验，
    让 LLM 同时看到正反两面的经验（Reflexion 启发）。
    """

    def __init__(self, max_size: int = 10, worst_size: int = 3):
        self._buffer: list[ParameterExperiment] = []
        self._worst_buffer: list[ParameterExperiment] = []  # R9-F: 最差实验
        self._max_size = max_size
        self._worst_size = worst_size
        self._violation_notes: list[str] = []  # R9-F: 违规反思笔记

    def add(self, experiment: ParameterExperiment) -> None:
        """添加一次实验记录，按收益排序保留 top-K，同时维护 worst-K。"""
        ...
        if len(self._buffer) > self._max_size:
            # R9-F: 淘汰的低效实验转入 worst_buffer
            evicted = self._buffer[self._max_size:]
            self._buffer = self._buffer[:self._max_size]
            for exp in evicted:
                self._add_to_worst(exp)
        # 同时检查当前实验是否属于最差的
        self._add_to_worst(experiment)

    def _add_to_worst(self, experiment: ParameterExperiment) -> None:
        """维护 worst-K 缓冲区（按净效益升序，最差的在前）。"""
        net = experiment.result.get("income", 0) - experiment.result.get("penalty_risk", 0)
        # 只保留净效益为负或较差的实验
        if net >= 0 and len(self._worst_buffer) >= self._worst_size:
            return
        # 去重：同一天的实验只保留一个
        self._worst_buffer = [e for e in self._worst_buffer if e.day != experiment.day]
        self._worst_buffer.append(experiment)
        self._worst_buffer.sort(
            key=lambda x: x.result.get("income", 0) - x.result.get("penalty_risk", 0))
        if len(self._worst_buffer) > self._worst_size:
            self._worst_buffer = self._worst_buffer[:self._worst_size]

    def add_violation_note(self, note: str) -> None:
        """R9-F: 添加违规反思笔记（由 daily_review 触发）。"""
        self._violation_notes.append(note)
        # 只保留最近 5 条
        if len(self._violation_notes) > 5:
            self._violation_notes = self._violation_notes[-5:]

    def to_prompt_context(self, top_k: int = 5) -> str:
        """生成 OPRO 风格的 few-shot context 供 LLM 参考。

        R9-F 增强：同时展示 top-K 成功实验和 worst-K 失败实验 + 违规反思。
        """
        if not self._buffer:
            return "（首次运行，暂无历史经验）"
        ...
        # R9-F: 展示失败实验（Reflexion）
        if self._worst_buffer:
            lines.append("")
            lines.append("失败实验（应回避这些参数组合）：")
            for i, exp in enumerate(self._worst_buffer):
                net = exp.result.get("income", 0) - exp.result.get("penalty_risk", 0)
                lines.append(
                    f"  ✗ Day{exp.day}: aggression={exp.params_snapshot.get('aggression', 0):.2f}, "
                    f"wait_threshold={exp.params_snapshot.get('wait_score_threshold', 0):.1f} "
                    f"→ net≈{net:.0f} (penalty_risk={exp.result.get('penalty_risk', 0):.0f})")

        # R9-F: 展示违规反思
        if self._violation_notes:
            lines.append("")
            lines.append("近期违规反思：")
            for note in self._violation_notes[-3:]:
                lines.append(f"  ⚠ {note}")
...
class StrategyAdvisor:
    ...
    v7 增强（ExperienceTracker 集成）：
      - daily_review 注入决策经验摘要（按时段×区域的收益/等待分布）
      - 让 LLM 看到细粒度的"哪类决策赚钱、哪类亏钱"
    """
    ...
    def __init__(self) -> None:
        ...
        # EvolveR: 策略原则存储（每个司机独立，最多保留 5 条最新原则）
        self._strategic_principles: dict[str, list[str]] = {}
        ...
        # ExperienceTracker 引用（由 ModelDecisionService 注入）
        self._experience_tracker: Any = None
    ...
    def daily_review(self, driver_id: str, state: DriverState,
        ...
        # R9-F: 生成违规反思笔记并注入
        violation_note = self._generate_violation_reflection(driver_id, state, config)
        if violation_note:
            exp_buf.add_violation_note(violation_note)

        # ExperienceTracker：注入决策经验摘要
        if self._experience_tracker is not None:
            decision_exp_summary = self._experience_tracker.generate_daily_summary(
                driver_id, current_day)
            review_context["decision_experience"] = decision_exp_summary

        # EvolveR: 注入已有策略原则供 LLM 参考/修正
        existing_principles = self._strategic_principles.get(driver_id, [])
        if existing_principles:
            review_context["existing_principles"] = existing_principles
        ...
        try:
            ...
            if choices:
                ...
                # EvolveR: 提取并存储策略原则
                new_principles = updates.get("principles", [])
                if new_principles and isinstance(new_principles, list):
                    # 去重并保留最新的 5 条原则
                    all_principles = new_principles + existing_principles
                    # 简单去重（精确匹配）
                    seen = set()
                    deduped = []
                    for p in all_principles:
                        if isinstance(p, str) and p not in seen:
                            seen.add(p)
                            deduped.append(p)
                    self._strategic_principles[driver_id] = deduped[:5]
                    logger.info(
                        "EvolveR day=%d: stored %d principles for %s",
                        current_day, len(self._strategic_principles[driver_id]), driver_id)
    ...
    def _build_decision_context(self, state: DriverState, config: DriverConfig,
        ...
        R9-E 增强：注入 ExperienceTracker 中"同时段同区域"的历史决策
        作为 few-shot 示例，让 LLM 看到过去类似场景的决策结果。
        """
        context = {
            "sim_minutes": state.sim_minutes,
            "current_day": state.current_day(),
        ...
        # ReflAct: 注入反思辅助信息——让 LLM 锚定"目标 vs 现实"差距
        days_remaining = max(1, 31 - state.current_day())
        net_income = state.total_gross_income - state.total_distance_km * state.cost_per_km
        context["reflection_context"] = {
            "days_elapsed": state.current_day(),
            "days_remaining": days_remaining,
            "net_income_so_far": round(net_income, 0),
            "avg_daily_net": round(net_income / max(1, state.current_day()), 0),
            "rest_satisfied_today": state.rest_satisfied_today,
            "off_days_count": len(state.off_days),
            "deadhead_ratio": round(
                state.total_deadhead_km / max(1, state.total_distance_km), 3),
        }

        # EvolveR: 注入策略原则（由 daily_review 蒸馏产出）
        principles = self._strategic_principles.get(state.driver_id, [])
        if principles:
            context["strategic_principles"] = principles

        # MUSE Semantic 层：注入从 Episodic 经验提炼的结构化语义记忆
        if self._experience_tracker is not None:
            semantic_memories = self._experience_tracker.extract_semantic_memory(
                state.driver_id, state.current_day())
            if semantic_memories:
                # 只注入 pattern 文本（节省 Token），按 confidence 降序取 top-3
                semantic_memories.sort(key=lambda m: m.get("confidence", 0), reverse=True)
                context["semantic_memory"] = [
                    m["pattern"] for m in semantic_memories[:3]
                ]

        # R9-E: 注入历史决策经验（few-shot）
        # ExpWeaver 门控：只在候选分差小（规则层"拿不准"）时才注入经验，避免噪声
        # 分差大说明规则层已有清晰判断，注入经验反而干扰 LLM
        scores = sorted([c.get("score", 0) for c in candidates], reverse=True)
        score_gap = (scores[0] - scores[1]) if len(scores) >= 2 else 999
        experience_gate_open = score_gap < 15.0  # 分差阈值：15分以内才注入

        if self._experience_tracker is not None and experience_gate_open:
            from agent.scoring.experience_tracker import hour_to_time_slot, pos_to_region_key
            current_ts = hour_to_time_slot(state.hour_in_day())
            current_rk = pos_to_region_key(state.current_lat, state.current_lng)
            exp_summary = self._experience_tracker.query_experience(
                state.driver_id, current_ts, current_rk, state.current_day())
            if exp_summary and exp_summary.count >= 3:
                context["historical_experience"] = {
                    "same_timeslot_region_stats": {
                        "sample_count": exp_summary.count,
                        "avg_income": round(exp_summary.avg_income, 0),
                        "avg_next_wait_minutes": round(exp_summary.avg_next_wait, 0),
                        "avg_score": round(exp_summary.avg_score, 1),
                        "confidence": round(exp_summary.confidence, 2),
                    },
                    "hint": "参考历史经验：该时段该区域的平均收入和等待时间，"
                            "高于均值的货值得接，低于均值的可以等更好的。"
                }

            # R10-A-v2: 注入 top-3 多样化历史经验 + 置信度
            top_exps = self._experience_tracker.query_top_experiences(
                state.driver_id, current_ts, current_rk, state.current_day(), top_k=3)
            if top_exps:
                context["top_historical_decisions"] = {
                    "experiences": [
                        {
                            "confidence": exp.confidence,
                            "net_income": exp.net_income,
                            "actual_income": round(exp.actual_income, 0),
                            "cargo_price": round(exp.cargo_price, 0),
                            "pickup_km": round(exp.pickup_km, 1),
                            "score_at_decision": round(exp.score_at_decision, 1),
                            "next_order_wait_minutes": round(exp.next_order_wait_minutes, 0),
                            "delivery_region": exp.delivery_region_key,
                            "day": exp.day,
                        }
                        for exp in top_exps
                    ],
                    "hint": "这是该时段该区域置信度最高的历史决策，按 confidence 降序排列。"
                            "置信度高的经验更值得参考，但不要完全忽略低置信度的多样性。"
                }

            # 同时检查卸货区域经验（如果候选货源有卸货位置）
            delivery_hints = []
            for c in candidates[:3]:
                d_lat = float(c.get("delivery_lat", 0))
                d_lng = float(c.get("delivery_lng", 0))
                if d_lat != 0 or d_lng != 0:
                    d_rk = pos_to_region_key(d_lat, d_lng)
                    d_summary = self._experience_tracker.query_delivery_region(
                        state.driver_id, current_ts, d_rk, state.current_day())
                    if d_summary and d_summary.count >= 3:
                        delivery_hints.append({
                            "cargo_id": c.get("cargo_id"),
                            "delivery_region_avg_income": round(d_summary.avg_income, 0),
                            "delivery_region_avg_wait": round(d_summary.avg_next_wait, 0),
                        })
            if delivery_hints:
                context["delivery_region_experience"] = delivery_hints
        elif self._experience_tracker is not None and not experience_gate_open:
            # 门控关闭：记录日志，不注入经验（规则层已有清晰判断）
            logger.debug("ExpWeaver gate closed: score_gap=%.1f >= 15, skip experience injection",
                         score_gap)
    ...
    def _generate_violation_reflection(
        self, driver_id: str, state: DriverState, config: DriverConfig,
    ) -> str | None:
        """R9-F: 纯规则生成违规/风险反思笔记（零 Token 消耗）。

        检查昨日状态与约束的匹配度，把接近违规或已违规的维度
        转换为一段自然语言「反思」，注入 ExperienceBuffer，
        让 LLM 在后续 daily_review 时看到并避免同类问题。
        ...
        if config.min_continuous_rest_minutes > 0:
            if state.longest_rest_today < config.min_continuous_rest_minutes:
                gap = config.min_continuous_rest_minutes - state.longest_rest_today
                issues.append(
                    f"昨日最长连续休息{state.longest_rest_today}分钟，"
                    f"差{gap}分钟不满足{config.min_continuous_rest_minutes}分钟要求，"
                    f"扣分风险约{config.penalty_weights.get('rest', 300):.0f}元"
                )

        # --- 2) off-day 进度落后 ---
        if config.monthly_off_days_required > 0:
            expected = config.monthly_off_days_required * days_elapsed / 31
            actual = len(state.off_days)
            if actual < expected - 0.5:
                deficit = config.monthly_off_days_required - actual
                days_left = 31 - day
                issues.append(
                    f"off-day 进度落后：已完成{actual}天，"
                    f"期望≈{expected:.1f}天，还差{deficit}天需在剩余{days_left}天内完成，"
                    f"单次违规扣分{config.penalty_weights.get('off_days', 3000):.0f}元"
                )

        # --- 3) 空驶超标趋势 ---
        if config.max_monthly_deadhead_km is not None:
            budget = config.max_monthly_deadhead_km
            used = state.total_deadhead_km
            ratio_used = used / max(1, budget)
            ratio_time = days_elapsed / 31
            if ratio_used > ratio_time * 1.2:
                remaining = budget - used
                issues.append(
                    f"空驶预算消耗过快：已用{used:.0f}/{budget:.0f}km "
                    f"({ratio_used:.0%})，时间进度{ratio_time:.0%}，"
                    f"剩余{remaining:.0f}km需撑{31 - day}天，"
                    f"应减少空驶接单"
                )

        # --- 4) 目标点到访落后 ---
        if config.visit_days_required > 0:
            expected_visits = config.visit_days_required * days_elapsed / 31
            actual_visits = len(state.visit_target_days)
            if actual_visits < expected_visits - 0.5:
                deficit = config.visit_days_required - actual_visits
                issues.append(
                    f"目标点到访落后：已{actual_visits}次，"
                    f"期望≈{expected_visits:.1f}次，还差{deficit}次，"
                    f"扣分风险{config.penalty_weights.get('visit_target', 5000):.0f}元/次"
                )

        # --- 5) 家事事件风险 ---
        if config.family_event is not None and state.family_phase not in ("done", "idle"):
            if state.family_phase in ("go_spouse", "wait_spouse", "go_home"):
                issues.append(
                    f"家事事件进行中(phase={state.family_phase})，"
                    f"未按时完成将面临{config.family_event.penalty_once_if_failed:.0f}元罚分，"
                    f"务必优先处理家事"
                )

        # --- 6) 首单 deadline 超时 ---
        if config.first_order_deadline_hour is not None:
            deadline_min = config.first_order_deadline_hour * 60
            if (state.today_first_order_minute is not None
                    and state.today_first_order_minute > deadline_min):
                late = state.today_first_order_minute - deadline_min
                issues.append(
                    f"首单超时{late}分钟（deadline={config.first_order_deadline_hour}:00），"
                    f"扣分{config.penalty_weights.get('first_order', 500):.0f}元，"
                    f"应更早开始接单"
                )

        # --- 7) 每日接单超限 ---
        if config.max_daily_orders is not None:
            if state.today_order_count > config.max_daily_orders:
                over = state.today_order_count - config.max_daily_orders
                issues.append(
                    f"昨日接单{state.today_order_count}笔超限{over}笔"
                    f"（上限{config.max_daily_orders}），每笔超限扣分"
                    f"{config.penalty_weights.get('max_orders', 500):.0f}元"
                )

        if not issues:
            return None

        header = f"[Day {day} 违规反思] 发现 {len(issues)} 个风险点："
        body = "；".join(issues) + "。"
